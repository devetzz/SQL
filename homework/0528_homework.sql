...
SELECT FIRST_NAME, SALARY, DEPARTMENT_ID FROM EMPLOYEES WHERE SALARY > (SELECT MIN(SALARY) FROM EMPLOYEES WHERE JOB_ID = 'ST_MAN') AND (DEPARTMENT_ID != 20 OR DEPARTMENT_ID IS NULL);
...
WHERE SALARY > (SELECT AVG(SALARY) FROM EMPLOYEES WHERE FIRST_NAME = 'Valli' GROUP BY DEPARTMENT_ID);